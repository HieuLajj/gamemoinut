...
public class Cilce8 : Board_Item
{
    public override bool Define_intersection(Vector3 positionM)
    {
        ...
        float radiuscicle = Vector3.Distance(transform.position,positionAnchor[5].position);
        if (distance < radiuscicle)
        {
            float distance2 = Vector3.Distance(transform.position, positionM);
            
            float radiuscicle2 = Vector3.Distance(transform.position, positionAnchor[4].position);
